...
/**
 * LightPillar (React Bits) — an animated volumetric light-pillar background.
 * Adapted to TypeScript; shader unchanged. Used as Wraith's ambient app backdrop.
 */
...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const isLowEndDevice =
      isMobile || (typeof navigator.hardwareConcurrency === 'number' && navigator.hardwareConcurrency <= 4)
  ...
  useEffect(() => {
    if (materialRef.current) materialRef.current.uniforms.uTopColor.value = new THREE.Color(topColor)
  }, [topColor])
  useEffect(() => {
    if (materialRef.current) materialRef.current.uniforms.uBottomColor.value = new THREE.Color(bottomColor)
  }, [bottomColor])
  useEffect(() => {
